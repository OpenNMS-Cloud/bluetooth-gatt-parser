...
public class BluetoothGattParser {
    ...
    private final String TAG = BluetoothGattParser.class.getSimpleName();
    ...
    public boolean validate(GattRequest gattRequest) {
        ...
        if (requirement != null) {
            ...
            if (required.isEmpty()) {
                Log.i(TAG, "GATT request is invalid; could not find any field by requirement: {}", requirement);
                return false;
            }
            for (FieldHolder holder : required) {
                if (!holder.isValueSet()) {
                    Log.i(TAG, "GATT request is invalid; field is not set: {}", holder.getField().getName());
                    return false;
                }
            }
        }
